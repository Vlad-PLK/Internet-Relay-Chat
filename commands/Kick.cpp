...
void    kick(User &user, Channel &channel_void, SocketServer &server, std::vector<std::string> &params)
{
    ...
    if (!server.findChannel(channel_title))
        ...
        user.usr_send((ERR_CHANOPRIVSNEEDED(user.getNickname(), server.getChannel(channel_title)->getTitle())).c_str());
    ...
    for (std::vector<std::string>::iterator it = users.begin(); it != users.end(); ++it)
    {
        if (!channel->userIsMember(*it) && !channel->userIsOperator(*it) && !channel->userIsOperator('@' + *it))
            user.usr_send((ERR_USERNOTINCHANNEL(*it, channel->getTitle())).c_str());
        else
        {
            std::string comment;
            if (!params[2].empty())
            ...
            for (std::vector<User>::iterator itUser = channel->getChannelUsers().begin(); itUser != channel->getChannelUsers().end(); ++itUser)
                    itUser->usr_send((RPL_KICK((user.getNickname() + "!" + user.getUsername() + "@localhost"), channel->getTitle(), *it, comment)).c_str());
            for (std::vector<User>::iterator itOp = channel->getChannelOperators().begin(); itOp != channel->getChannelOperators().end(); ++itOp)
                    itOp->usr_send((RPL_KICK((user.getNickname() + "!" + user.getUsername() + "@localhost"), channel->getTitle(), *it, comment)).c_str());
